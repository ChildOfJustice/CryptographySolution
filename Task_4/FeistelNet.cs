...
namespace Task_4
{
    public class FeistelNet
    {
        ...
        static uint FeistelFunction(uint R, ulong RoundKey)
        {
        ...
        public byte[] Cipher(byte[] DataBytes, bool Way)
        {
	        if (DataBytes.Length != 8)
	        {
		        throw new ArgumentException("DataBytes");
	        }
	        //ulong Data = Task_1.Program.Permute(BitConverter.ToUInt64(DataBytes, 0), InitialPermutation);
	        ...
	        return BitConverter.GetBytes(((ulong)L << 32) | R);
        }
    }
}